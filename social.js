...
(() => {
  ...
  function authModal(startSignup=false){
  ...
  async function loadUser(){
    ...
    if(login){login.textContent=currentUser?'CERRAR SESIÓN':'INICIAR SESIÓN';login.onclick=async()=>currentUser?(await sb.auth.signOut(),await loadUser()):authModal(false)}
    const hb=$('#headerLoginBtn'); if(hb) hb.textContent=currentUser?'SALIR':'ENTRAR';
  ...
  function setupWelcome(){
    const hide=()=>{document.body.classList.add('welcome-dismissed');window.scrollTo({top:0,behavior:'smooth'});};
    $('#welcomeSkip')?.addEventListener('click',hide);
    $('#welcomeSignup')?.addEventListener('click',()=>authModal(true));
    $('#welcomeLogin')?.addEventListener('click',()=>authModal(false));
    $('#heroJoin')?.addEventListener('click',()=>authModal(false));
    $('#headerLoginBtn')?.addEventListener('click',()=>currentUser?(sb.auth.signOut().then(loadUser)):authModal(false));
  }

  function setupWelcomeCarousel(){
    const root=$('#welcomeCarousel'); if(!root) return;
    const slides=[...root.querySelectorAll('.welcome-slide')]; const dots=root.querySelector('#carouselDots');
    let index=0, timer=null;
    if(!slides.length) return;
    slides.forEach((_,i)=>{const d=document.createElement('button');d.className='carousel-dot'+(i===0?' active':'');d.type='button';d.setAttribute('aria-label',`Ir a imagen ${i+1}`);d.addEventListener('click',()=>go(i,true));dots?.appendChild(d);});
    const dotEls=[...root.querySelectorAll('.carousel-dot')]; const counter=$('#carouselCounter');
    function go(next,manual=false){index=(next+slides.length)%slides.length;slides.forEach((el,i)=>el.classList.toggle('is-active',i===index));dotEls.forEach((el,i)=>el.classList.toggle('active',i===index));if(counter)counter.textContent=`${String(index+1).padStart(2,'0')} / ${String(slides.length).padStart(2,'0')}`;if(manual)restart();}
    function restart(){clearInterval(timer);timer=setInterval(()=>go(index+1),5500);}
    $('#carouselPrev')?.addEventListener('click',()=>go(index-1,true));$('#carouselNext')?.addEventListener('click',()=>go(index+1,true));
    root.addEventListener('mouseenter',()=>clearInterval(timer));root.addEventListener('mouseleave',restart);restart();
  }
