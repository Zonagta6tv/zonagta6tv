...
if(el("#blogCards")) el("#blogCards").innerHTML=BLOG.map(x=>`<article class="story"><span>${x[0]}</span><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join("");
if(el("#guideCards")) el("#guideCards").innerHTML=GUIDES.map(x=>`<article class="guide"><div class="n">${x[0]}</div><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join("");
if(el("#leakCards")) el("#leakCards").innerHTML=LEAKS.map((x,i)=>`<article class="leak"><div class="confidence ${i===0?"high":i===1?"mid":"low"}">● CONFIANZA ${x[0]}</div><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join("");

const open=id=>el("#"+id).classList.add("open"), close=id=>el("#"+id).classList.remove("open");
document.querySelectorAll("[data-close]").forEach(b=>b.onclick=()=>close(b.dataset.close));
if(el("#openSearch")) el("#openSearch").onclick=()=>open("searchModal"); if(el("#heroSearch")) el("#heroSearch").onclick=()=>open("searchModal");
if(el("#searchInput")) el("#searchInput").addEventListener("input",e=>{const q=e.target.value.trim().toLowerCase(),out=el("#results");if(!q){out.innerHTML="";return}const f=NEWS.filter(n=>n.join(" ").toLowerCase().includes(q));out.innerHTML=f.length?f.map(n=>`<div class="search-item"><b>${esc(n[2])}</b><div style="color:#8e92a5;font-size:11px">${esc(n[3])}</div></div>`).join(""):"<p style='color:#787d90'>No encontramos resultados.</p>"});
...
if(audio) audio.volume=.18;
if(audioBtn) audioBtn.onclick=async()=>{try{if(audio.paused){await audio.play();audioBtn.classList.add("active");audioBtn.textContent="🔊";toast("Ambiente sonoro activado.");}else{audio.pause();audioBtn.classList.remove("active");audioBtn.textContent="♫";toast("Ambiente pausado.");}}catch{toast("Pulsa de nuevo para activar el audio.")}};
...
function fpsTick(t){frames++;if(t-last>500){if(el("#fps")) el("#fps").textContent=Math.min(60,Math.round(frames*1000/(t-last)));frames=0;last=t}requestAnimationFrame(fpsTick)}requestAnimationFrame(fpsTick);

const target=new Date("2026-11-19T00:00:00-05:00");
function tick(){let s=Math.max(0,Math.floor((target-new Date())/1000)),d=Math.floor(s/86400);s%=86400;let h=Math.floor(s/3600);s%=3600;let m=Math.floor(s/60),z=s%60;if(el("#bigCountdown")) el("#bigCountdown").textContent=`${d}d ${String(h).padStart(2,"0")}h ${String(m).padStart(2,"0")}m ${String(z).padStart(2,"0")}s`}tick();setInterval(tick,1000);
if(el("#themeBtn")) el("#themeBtn").onclick=()=>{document.body.classList.toggle("light");el("#modeLabel").textContent=document.body.classList.contains("light")?"VIOLET SUNSET":"NEON NIGHT";toast("Ambiente actualizado.")};
...
if(el("#quizBtn")) el("#quizBtn").onclick=()=>{qi=0;score=0;quiz();open("quizModal")};
...
let ti=0; setInterval(()=>{ti=(ti+1)%tickers.length;if(el("#tickerText")) el("#tickerText").textContent=tickers[ti]},3200);
setInterval(()=>{const h=80+Math.floor(Math.random()*18),a=["HIGH","MAX","ACTIVE"][Math.floor(Math.random()*3)];if(el("#heatValue")) el("#heatValue").textContent=h+"%";if(el("#activityValue")) el("#activityValue").textContent=a},1800);
