...
function renderNews(filter="todos"){const images=["news-01-extended-look.svg","news-02-launch.svg","news-03-leonida.svg","news-04-jason-lucia.svg","news-05-gallery.svg","news-06-community.svg"];const labels=["VI","19 NOV","LEÓNIDA","J+L","MEDIA","ZONA"];el("#newsCards").innerHTML=NEWS.filter(n=>filter==="todos"||n[5]===filter).map((n,i)=>{const originalIndex=NEWS.indexOf(n);return `<article class="card news-card" data-category="${n[5]}"><div class="art"><img src="assets/${images[originalIndex]}" alt="${esc(n[2])}" loading="lazy"><span class="art-glow"></span><b>${labels[originalIndex]}</b></div><div class="card-body"><div class="meta">${n[0]} · ${n[1]}</div><h3>${n[2]}</h3><p>${n[3]}</p><button class="read" data-article="official">ABRIR ARTÍCULO →</button></div></article>`}).join("")}renderNews();
...
// Comentarios públicos sin obligar a tener GitHub: Disqus con comentarios de invitado.
// IMPORTANTE: crea el sitio en Disqus con este shortname y activa Guest Commenting.
const DISQUS_SHORTNAME="zonagta6tv";
if(DISQUS_SHORTNAME && !DISQUS_SHORTNAME.startsWith("TU_")){
  window.disqus_config=function(){this.page.url=window.location.href;this.page.identifier=location.pathname;this.page.title=document.title;};
  const s=document.createElement("script");s.src="https://"+DISQUS_SHORTNAME+".disqus.com/embed.js";s.setAttribute("data-timestamp",+new Date());s.async=true;s.setAttribute("crossorigin","anonymous");el("#utterances").replaceChildren(s);
}
